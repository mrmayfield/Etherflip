...
enum Stages {
    ...
    function reveal( uint256 seedBUserParam2 ) atStage(Stages.Reveal) returns (uint256){
        ...
        //limit to one game at a time for alpa phase. beta:1 to many
            ...
            if(seedBStage2Hash == seedBStage1Hash){
                ...
                if(high==true && seedB > 49){
                    ...
                    for (i = 0; i < arrayLength; i++) {
                        ...
                        uint playerNumber = funders[i].Number;
                        uint theAmount = funders[i].amount;
                        uint theReward =  funders[i].rewardValue;
                        address theAddress = funders[i].addr;
    ...
    function payEntrants (uint randResult) constant {

        uint randomResult = randResult;
        uint arrayLength = funders.length;
        uint i;
        ...
        for (i = 0; i < arrayLength; i++) {
            ...
            uint playerNumber = funders[i].Number;
            uint theAmount = funders[i].amount;
            uint theReward =  funders[i].rewardValue * 5;
            address theAddress = funders[i].addr;
            //address theAddr = funders[i].addr;
            ...
            if(playerNumber > randomResult){
                theAddress.send(theReward);
            }



            //return theNumber + theReward;
        }
